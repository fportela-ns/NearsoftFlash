...
const EmptyContainer = props => <div className={props.style}>{props.text}</div>;

const withEmptyContainer = (Component, text, style) => props =>
  props.currentSlide === null ? (
    <EmptyContainer text={text} style={style} />
  ) : (
    <Component {...props} />
  );

const CanvasWithConditionalRendering = withEmptyContainer(
  Canvas,
  'Empty Canvas, Add new Slide to Begin...',
  'slide-canvas--container-empty'
);
const SlideConfigurationWithConditionalRendering = withEmptyContainer(
  SlideConfiguration,
  'Empty config.',
  'slide-configuration--empty'
);
...
class NSFlash extends React.Component {
  ...
  onAddSlide() {
    ...
    const slides = slides.map(
      s => (currentSlide.id === s.id ? currentSlide : s)
    );
    this.setState({
      slides: [slide, ...slides],
      currentSlide: slide
  ...
  onSlideSelect(e, id) {
    e.preventDefault();
    const slides = this.state.slides.map(
      s => (this.state.currentSlide.id === s.id ? this.state.currentSlide : s)
    );
    const selectedSlide = this.state.slides.find(s => s.id === id);
  ...
  onSelectContainerSevenGeese() {
    if (this.state.currentSlide.sevenGeese === null) {
      ...
      this.setState({
        currentSlide: Object.assign(this.state.currentSlide, {
          sevenGeese,
          active: 'sevenGeese'
        })
      });
  ...
  onDragStopConfigEvent(e, container, property) {
    ...
    const currentSlide = Object.assign({}, this.state.currentSlide);
    currentSlide[container][property].positionX = parseInt(
      e.target.style.transform.match(/([0-9]){1,5}/g)[0]
    );
    currentSlide[container][property].positionY = parseInt(
      e.target.style.transform.match(/([0-9]){1,5}/g)[1]
    );
    this.setState({ currentSlide });
  }
  ...
  onResize() {
    ...
    const currentSlide = Object.assign({}, this.state.currentSlide);
    currentSlide.main.title.positionX = Math.ceil(
      (currentWidth * currentSlide.main.title.positionX) / this.state.width
    );
    currentSlide.main.title.positionY = Math.ceil(
      (currentHeight * currentSlide.main.title.positionY) / this.state.height
    );
    currentSlide.main.description.positionX = Math.ceil(
      (currentWidth * currentSlide.main.description.positionX) /
        this.state.width
    );
    currentSlide.main.description.positionY = Math.ceil(
      (currentHeight * currentSlide.main.description.positionY) /
        this.state.height
    );
    this.setState({
      currentSlide,
    ...
    return (
      <div className="main-container">
        ...
        <CanvasWithConditionalRendering
          ...
          onSelectContainerSevenGeese={this.onSelectContainerSevenGeese.bind(
            this
          )}
        ...
        <SlideConfigurationWithConditionalRendering
          handleConfigInputChangeForMainContainer={this.handleConfigInputChangeForMainContainer.bind(
            this
          )}
